#!/usr/bin/env python3
"""
Automated SAST Scanner using Semgrep
=====================================

This script performs automated security scanning on the src/ directory
using Semgrep and generates comprehensive reports in the SAST/ folder.
...
Output:
    - SAST/semgrep_report.json (JSON format)
    - SAST/semgrep_report.txt (Human-readable format)
    - SAST/semgrep_report.html (HTML format)
    - SAST/semgrep_summary.md (Markdown summary)
"""
...
SAST_DIR = SCRIPT_DIR
CUSTOM_RULES = SAST_DIR / "custom-rules.yaml"
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")

# Semgrep rulesets to use - Multiple comprehensive rulesets
# Starting with custom rules only, then will add public rulesets
RULESETS = []

# Public rulesets (uncomment after custom rules work)
PUBLIC_RULESETS = [
    "p/react",
    "p/javascript", 
    "p/typescript",
...
# Output files
REPORT_JSON = SAST_DIR / f"semgrep_report_{TIMESTAMP}.json"
REPORT_TXT = SAST_DIR / f"semgrep_report_{TIMESTAMP}.txt"
REPORT_HTML = SAST_DIR / f"semgrep_report_{TIMESTAMP}.html"
REPORT_SARIF = SAST_DIR / f"semgrep_report_{TIMESTAMP}.sarif"
SUMMARY_MD = SAST_DIR / "semgrep_summary.md"

# Latest report symlinks (tanpa timestamp)
LATEST_JSON = SAST_DIR / "semgrep_report_latest.json"
LATEST_TXT = SAST_DIR / "semgrep_report_latest.txt"
LATEST_HTML = SAST_DIR / "semgrep_report_latest.html"
...
╚════════════════════════════════════════════════════════════╝
{Colors.ENDC}
{Colors.OKCYAN}Target Directory:{Colors.ENDC} {SRC_DIR}
{Colors.OKCYAN}Output Directory:{Colors.ENDC} {SAST_DIR}
{Colors.OKCYAN}Timestamp:{Colors.ENDC} {TIMESTAMP}
{Colors.OKCYAN}Rulesets:{Colors.ENDC} {', '.join(RULESETS)}
"""
    print(banner)
...
def ensure_directories():
    ...
    if not SRC_DIR.exists():
        print(f"{Colors.FAIL}✗ Source directory not found: {SRC_DIR}{Colors.ENDC}")
        return False
    print(f"{Colors.OKGREEN}✓ Source directory found: {SRC_DIR}{Colors.ENDC}")
    return True


def run_semgrep_scan():
    """Run Semgrep scan with multiple output formats"""
    print(f"\n{Colors.OKBLUE}[3/6] Running Semgrep scan...{Colors.ENDC}")
    ...
    if CUSTOM_RULES.exists():
        ...
        configs_used.append(f"custom-rules.yaml")
        print(f"{Colors.OKCYAN}✓ Using custom rules: {CUSTOM_RULES.name}{Colors.ENDC}")
    else:
        print(f"{Colors.WARNING}⚠ Custom rules not found, skipping{Colors.ENDC}")
    
    # Add each public ruleset separately
    for ruleset in RULESETS:
        cmd.extend(["--config", ruleset])
        configs_used.append(ruleset)
    
    if configs_used:
        print(f"{Colors.OKCYAN}Configs: {', '.join(configs_used)}{Colors.ENDC}")
    else:
        print(f"{Colors.FAIL}✗ No configurations specified!{Colors.ENDC}")
        return False
...
def generate_text_report():
    ...
    try:
        ...
        with open(REPORT_TXT, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("SEMGREP SECURITY SCAN REPORT\n")
            f.write("=" * 80 + "\n\n")
            f.write(f"Scan Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Target: {SRC_DIR}\n")
            f.write(f"Rulesets: {', '.join(RULESETS)}\n")
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .finding-message {{ color: #555; margin-bottom: 15px; line-height: 1.6; }}
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Target: {SRC_DIR.relative_to(PROJECT_ROOT)}</p>
        </div>
...
"""
        ...
        <div class="footer">
            Generated by Automated SAST Scanner using Semgrep
        </div>
    </div>
</body>
</html>
...
def generate_markdown_summary():
    ...
    try:
        ...
        md_content = f"""# Semgrep SAST Scan Summary

**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  
**Target:** `{SRC_DIR.relative_to(PROJECT_ROOT)}`  
...
def print_summary():
    ...
    print(f"{Colors.HEADER}{Colors.BOLD}SCAN COMPLETED SUCCESSFULLY{Colors.ENDC}")
