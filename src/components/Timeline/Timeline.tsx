...
export function Timeline({ projectId }: TimelineProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'timeline_events', filter: `project_id=eq.${projectId}` },
        () => {
          loadEvents();
        }
      )
  ...
  const getEventColor = (type: string) => {
    const colors = {
      ...
      'message': 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400',
  ...
  const getEventDescription = (event: TimelineEvent) => {
    ...
    switch (event_type) {
      case 'project':
        if (event_action === 'created') return `created the project "${data.name}"`;
      ...
      case 'task':
        ...
        if (event_action === 'completed') return `completed task "${data.title}"`;
        if (event_action === 'updated') return `updated task "${data.title}" to ${data.status}`;
        if (event_action === 'deleted') return `deleted task "${data.title}"`;
        return `${event_action} a task`;

      case 'message':
        return `sent a message: "${data.content}${data.content?.length > 50 ? '...' : ''}"`;

      case 'file':
        if (event_action === 'uploaded') return `uploaded file "${data.file_name}"`;
  ...
  return (
    <div className="p-6">
      <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Activity Timeline</h3>
